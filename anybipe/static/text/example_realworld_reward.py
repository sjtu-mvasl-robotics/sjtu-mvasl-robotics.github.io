...
class RealworldReward:
    ...
    def _reward_base_height(self, idx):
            ...
            return torch.exp(-base_height_error / height_sigma) * 0.1


    def _reward_survival(self, idx):
        return 0.1 * self.env.dt[idx]

    

    def _reward_torque_limits(self, idx):
            env = self.env
            torque_penalty = torch.sum(
                (torch.abs(env.torques[idx:idx+1]) - env.torque_limits * self.reward.soft_torque_limit).clamp(min=0.), dim=1)
            return -0.01 * torque_penalty
